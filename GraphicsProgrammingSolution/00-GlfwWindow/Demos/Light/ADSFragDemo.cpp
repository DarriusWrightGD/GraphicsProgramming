#include "ADSFragDemo.h"
#include <assimp\postprocess.h>
#include <assimp\Importer.hpp>
#include <assimp\scene.h>


ADSFragDemo::ADSFragDemo()
{
}
...
void ADSFragDemo::Update()
{
	monkeyObject->Update();
}

void ADSFragDemo::Render()
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	renderer->Render();
}

void ADSFragDemo::OnResize(int width, int height)
{
	camera->SetAspectRatio(static_cast<float>(width) / static_cast<float>(height));
	program.UpdateUniformBlock("TransformBlock");
}

void ADSFragDemo::Initialize()
{
	...
	{
		monkeyMesh = std::unique_ptr<MeshComponent>(new MeshComponent(monkeyObject.get(), scene->mMeshes[0], program, [this](GLProgram & program) {
