#include "DiscardDemo.h"
#include <assimp\postprocess.h>
#include <assimp\Importer.hpp>
#include <assimp\scene.h>

DiscardDemo::DiscardDemo()
{
}
...
void DiscardDemo::Update()
{
	monkeyObject->Update();
}

void DiscardDemo::Render()
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	renderer->Render();
}

void DiscardDemo::OnResize(int width, int height)
{
	camera->SetAspectRatio(static_cast<float>(width) / static_cast<float>(height));
	program.UpdateUniformBlock("TransformBlock");
}

void DiscardDemo::Initialize()
{
	glEnable(GL_DEPTH_TEST);
	program.AddShaderFile(ShaderType::Vertex, "Assets/Shaders/Vertex/discard.vert");
	program.AddShaderFile(ShaderType::Fragment, "Assets/Shaders/Fragment/discard.frag");
	program.Build();
